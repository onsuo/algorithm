...
int main()
{
    ...
    int N;
    std::cin >> N;
    std::array<People, 50> peopleList;
    for (int i = 0 ; i < N ; i++) {
        std::cin >> peopleList[i].weight >> peopleList[i].height;
    }
    for (int i = 0 ; i < N ; i++) {
        for (int j = 0 ; j < N ; j++) {
            bool isBigger = peopleList[j].weight > peopleList[i].weight && peopleList[j].height > peopleList[i].height;
            if (isBigger) { peopleList[i].rank++; }
        }
        std::cout << peopleList[i].rank << " ";
    }
}